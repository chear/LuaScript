...
--~ author:chear
--~ Noted: print every package from received, stream package for hex stream.
--[[
...
--local SRC_FILENAME = [[/Users/test1/Documents/kirk_data/BTRecording16-30-10.txt]];
--local SRC_FILENAME = [[C:\Users\admin\Downloads\data\1\UartRecording-13-37-49.txt]];
--local SRC_FILENAME = [[C:\Users\admin\Downloads\data\SSave-01-33.txt]];
local SRC_FILENAME = [[E:\tmp-copy\tmp\SSave-02-04.txt]];
...
local checkout_sum_error_count=0;
...
for i,arg in ipairs( arg ) do
    ...
    while true do
		local bytes = file:read(block)
      	...
      	for b in string.gmatch(bytes, ".") do
      		i = i+1;
			word = string.format("%02X", string.byte(b));
			print("byte:"..word);
			...
			elseif( state == "CKSUM" ) then
				...
				if( cksum ~= (255-sum) ) then
					...
					checkout_sum_error_count=checkout_sum_error_count+1;
    ...
    print("----------------------");
	print("---- payload:"..payload_count.."-----");
	print("---- check sum error:"..checkout_sum_error_count.."-----");
